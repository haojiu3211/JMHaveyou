...
final class ChatBannerView: UIView {
    ...
    /// 原始数据
    private var items: [ChatBannerModel] = []

    /// 含首尾 ghost 帧的渲染数组：[last, item0, item1, ..., itemN, first]
    private var loopItems: [ChatBannerModel] = []
    ...
    /// Banner 点击回调，返回被点击的 ChatBannerModel（原始索引对应的真实数据）
    var onBannerTapped: ((ChatBannerModel) -> Void)?
    ...
    func configure(_ banners: [ChatBannerModel]) {
        stopAutoScroll()
        items = banners
    ...
    private func buildPages() {
        ...
        for (i, item) in loopItems.enumerated() {
            ...
            if let url = URL(string: AppConfig.API.fullImageURL(path: item.image)) {
                card.kf.setImage(with: url, placeholder: nil)
            } else {
                card.image = UIImage(named: item.image)
            }
...
extension ChatBannerView: UIScrollViewDelegate {
